...
- **The prompt goes to the harness on stdin, never as an argv string.** Linux caps a *single* argument at `MAX_ARG_STRLEN` — 32 pages, 128 KiB — independently of `ARG_MAX`, so `-p "$(<"$prompt")"` dies with `Argument list too long` once the staged diff passes that, on a host whose `ARG_MAX` is 2 MiB and where nothing else looks wrong. The failure surfaces as the harness never starting, which reads as a harness fault rather than an exec fault. `claude -p` with no positional prompt and `codex exec -` both read stdin; keep it that way, and do not "simplify" either call back into an argument.
